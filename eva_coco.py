...
def main():
    ...
    # TODO 
    # load vision transformer from rei/eva_clip/eva_vit_model.py
    ...
    teacher = teacher.to(device, dtype=dtype)
    del teacher.text
    del p
    ...
    for epoch in range(epochs):
        ...
        for i, (samples, targets, teacher_samples) in enumerate(data_loader_train):
            ...
            teacher_samples = torch.stack(processed_teacher_samples).to(f'cuda:{args.local_rank}', dtype=dtype)

            out = model_engine(samples)
            target = teacher.encode_image(teacher_samples)
